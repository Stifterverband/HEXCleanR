...
#' visualisiert diese als Balloon Plot. Die Kreisgroesse und -farbe kodieren die
#' Anzahl fehlender Werte, optional werden die absoluten Werte als Text in den
#' Kreisen angezeigt. List-Spalten werden vor der Umformung automatisch
#' ausgeschlossen.
#'
...
#' @param low_fill Farbe fuer niedrige NA-Anzahlen.
#' @param high_fill Farbe fuer hohe NA-Anzahlen.
...
plot_na_balloons <- function(
  ...
  na_long <- plot_data |>
    ...
    dplyr::summarise(n_na = sum(is.na(wert)), .groups = "drop")

  na_table <- na_long |>
    dplyr::arrange(.data[[grp_var_name]], dplyr::desc(.data$n_na), .data$variable)
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::geom_point(
      ggplot2::aes(size = .data$n_na, fill = .data$n_na),
      shape = 21,
      colour = "black",
      stroke = 0.25,
      alpha = 0.75
  ...
  if (isTRUE(show_labels)) {
    p <- p +
      ggplot2::geom_text(
        data = dplyr::filter(na_table, .data$n_na > 0),
        ggplot2::aes(label = .data$n_na),
        size = 3.5,
        colour = "white",
        fontface = "bold"
      )
