...
1.  Wenn nur ein `titel` vorliegt und `kursbeschreibung` fehlt, wird
    [`detect_lang_with_openai()`](detect_lang_with_openai.md) auf diese
    Zeilen angewendet.
...
- db_data_path:
  ...
  für [`detect_lang_with_openai()`](detect_lang_with_openai.md). Wenn
  `NULL` oder nicht vorhanden, wird ohne DB-Lookup gearbeitet.

- export_path:

  Pfad zum Sicherheits-Export für
  [`detect_lang_with_openai()`](detect_lang_with_openai.md). Standard
